...
const AUTHED = {
  ...
  "x-openclaw-device-id": "device-uuid-1",
  "x-openclaw-clerk-session-id": "sess_1",
};
...
describe("POST /gateway/mobile/signout (G-lane [G2])", () => {
  it("★ happy path: verifies JWT, unbinds the caller's OWN (shrinemobile, device) link, denies the sid, 200", async () => {
    ...
    expect(deps.evicted).toEqual(["sess_1"]); // [G2b] positive-cache window closed
  });
  ...
  it("★ missing bearer → 401, no unbind, no deny (fail-closed)", async () => {
    ...
    expect(deps.unlinkCalls).toHaveLength(0);
    expect(deps.evicted).toHaveLength(0);
  });
  ...
  it("still evicts on a NO-OP unbind (a second sign-out still closes a live session's cache)", async () => {
    const deps = makeDeps({ unlink: async () => 0 });
    const { res } = fakeRes();
    await createMobileSignoutHandler(deps)(fakeReq({ headers: AUTHED }), res);
    expect(deps.evicted).toEqual(["sess_1"]);
  });

  it("★ NO session-id header → unbinds fine but evicts nothing (evict is keyed on the header, not the JWT)", async () => {
    const deps = makeDeps();
    const { res, state } = fakeRes();
    // Authed + device id, but NO x-openclaw-clerk-session-id: unbind still runs,
    // there is simply no cache key to evict.
    await createMobileSignoutHandler(deps)(
      fakeReq({
        headers: { authorization: "Bearer aaa.bbb.ccc", "x-openclaw-device-id": "device-uuid-1" },
      }),
      res,
    );
    expect(state.status).toBe(200);
  ...
  it("★ DB failure → 500 (loud), but the sid IS already denied (consent-kill survives the outage)", async () => {
    ...
    expect(deps.evicted).toEqual(["sess_1"]); // evicted BEFORE the unlink attempt
  });
