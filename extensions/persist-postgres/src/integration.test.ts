 ...
 *
 * Requires a running PostgreSQL instance. Set the DATABASE_URL env var to run.
...
const sql = shouldRun ? postgres(DATABASE_URL, { max: 5 }) : (undefined as unknown as postgres.Sql);
...
// Wrap setup/teardown in a skippable block to prevent
// connecting to Postgres when DATABASE_URL is not set.
describe.skipIf(!shouldRun)("persist-postgres integration", () => {
  beforeAll(async () => {
  ...
  describe("PostgreSQL conversation CRUD", () => {
    ...
    `;
      ...
      // upsertConversation no longer increments message_count (handled by insertMessage)
