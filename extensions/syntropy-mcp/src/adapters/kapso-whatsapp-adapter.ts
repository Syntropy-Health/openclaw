 ...
 * added here would be content this channel is not cleared to carry). The switch
 * is exhaustive over the `DeliveryPayload` union, so adding a payload kind to the
 * frozen seam becomes a compile error here rather than a silent empty message.
 */
 ...
 *
 * Throws at CONSTRUCTION if the D0 lookup fails — see the guard below.
 */
export function createKapsoWhatsAppAdapter(deps: KapsoWhatsAppAdapterDeps): ChannelAdapter {
  ...
  if (!capabilities) {
    throw new Error(
      `kapso whatsapp adapter: no capability row for channel "${WHATSAPP_CHANNEL_ID}" — refusing to construct`,
    );
  }
   ...
   * escape `deliver()` as an exception.
   */
  ...
  const adapter: ChannelAdapter = {
     ...
     * bare-digit `from` to canonical `+E164`, and drops a sender that collapses to
     * the degenerate `"+"`. Re-implementing any of that here would give WhatsApp a
    ...
    identity(inbound: unknown): ChannelIdentity {
      ...
      if (!only) {
        throw new Error(
          `kapso whatsapp adapter: cannot derive a single identity — payload yielded ${inbounds.length} identifiable inbound message(s), expected exactly 1`,
        );
      }

      return { e164: only.from, channelId: WHATSAPP_CHANNEL_ID };
    },
    ...
    deliver(payload: DeliveryPayload): Promise<DeliveryResult> {
      return deliverViaCapabilities(adapter.capabilities, payload, {
        // Inline: send to the paired peer bound at construction.
        inline: (p) => send(deps.session.toE164, p),
        // Companion: send to the number the RESOLVER supplies, never to a
        // destination we picked. Unreachable for WhatsApp's all-inline row (see
        // the note above); present so the seam's shape is uniform across channels.
