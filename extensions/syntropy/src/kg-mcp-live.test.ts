 ...
 * shrine-diet-bioactivity submodule's own test suite + the KG-direct
 * sub-plan (GH #64 / SYN-33).
 */
...
describe.skipIf(!LIVE)("kg-mcp (LIVE)", () => {
  ...
  it("GET /health without Bearer returns 200, 401, or 403 (contract drift if other)", async () => {
    ...
    // Some gateways treat /health as unauthenticated — accept 200 or 401/403,
